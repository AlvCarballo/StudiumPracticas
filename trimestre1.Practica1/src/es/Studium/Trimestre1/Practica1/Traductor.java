...
public class Traductor 
{
	...
	public static void main(String[] args) 
	{
		...
		String tabla[] = new String[palabra.length()];
		String[] parts = palabra.split(" ");//split separa por lo que se le diga
		separarFrase(tabla, parts);
		System.out.println("La frase traducida es:");
		revisarTraductor(tabla, parts);
	}

	private static void separarFrase(String[] tabla, String[] parts) {
		for(int i=0;i<parts.length;i++)//recorremos tantas veces como palabras tenga la frase
		{
			tabla[i] = parts[i];//metemos las palabras en una tabla
		}
	}

	private static void revisarTraductor(String[] tabla, String[] parts) {
		for(int i=0;i<parts.length;i++)//recorremos tantas veces como palabras hay.
		{
			switch (tabla[i])//Miramos si las palabras almacenadas en la tabla y imprimimos el resultado
			{
			case "yo":
				System.out.print("i");
				System.out.print(" ");
